...
public class Stopcompetition extends JavaPlugin implements Listener {
	...
	public void onEnable() {
		...
		stopTime = getConfig().getInt("DelayStopCommand");
		notify = getConfig().getBoolean("NotifyPlayers");
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (commandLabel.equalsIgnoreCase("ssc")) {
			if (sender.hasPermission("stop.admin")) {
				gameEnabled = true;
				Bukkit.broadcastMessage("The game has started! Type /stop right now. The person who types it will win the game!");
			}
		}
